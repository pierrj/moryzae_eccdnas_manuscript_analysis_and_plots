---
title: "if_expression_large_eccdnas_analysis_infectedsrs"
...
ggsave("if_expression_large_eccdnas_rpkm_plot_infectedsrs.pdf", plot = p, width = 6, height = 4)
...
pdf(file = "if_expression_large_eccdnas_rpkm_diagnostics_infectedsrs.pdf", width = 6, height = 4)
par(mfrow=c(2,2))
plot(lm(df_rpkm$srs ~ df_rpkm$rpkm))
dev.off()
```
...
ggsave("if_expression_large_eccdnas_genesperk100kb_plot_infectedsrs.pdf", plot = p, width = 6, height = 4)
```
...
pdf(file = "if_expression_large_eccdnas_genesper100kb_diagnostics_infectedsrs.pdf", width = 6, height = 4)
par(mfrow=c(2,2))
plot(lm(df_genesper100kb$srs ~ df_genesper100kb$gene_count))
dev.off()
```
...
ggsave("if_expression_large_eccdnas_genesperk100kbperscaffold_plot_infectedsrs.pdf", plot = p, width = 6, height = 4)
```
...
pdf(file = "if_expression_large_eccdnas_genesperk100kbperscaffold_diagnostics_infectedsrs.pdf", width = 6, height = 4)
par(mfrow=c(2,2))
plot(lm(df_genesperscaffold$srs ~ df_genesperscaffold$gene_count))
dev.off()
```
